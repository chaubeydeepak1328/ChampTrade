...
const EarningsBreakdown = () => {
  ...
  const getMyEarningsData = useStore((state) => state.getMyEarningsData);

  useEffect(() => {
    const fetchEarning = async () => {
      const res = await getMyEarningsData(userAddress);
      setEarningData(res);
    }
    fetchEarning();
  }, [])
      ...
      <div className="space-y-4 px-1 sm:px-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          ...
          {/* <div className="bg-gradient-to-br from-[rgb(30,30,30)] to-[rgb(20,20,20)] p-3 sm:p-4 rounded-lg border border-yellow-500/10 hover:border-yellow-500/30 transition-colors">
            ...
            <p className="text-xs text-yellow-500/70 mt-1">{earningData?.unclaimedTCCCount}</p>
          </div> */}
        </div>
