        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="relative overflow-hidden p-16">
            {testimonials.map((testimonial, index) => (
              <div
                key={testimonial.id}
                className={`transition-all duration-500 absolute inset-0 flex flex-col md:flex-row items-center gap-8 p-8 ${index === current
            ...
            <div className="flex space-x-2">
              {testimonials.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${index === current ? 'bg-yellow-500 scale-125' : 'bg-yellow-500/30'
                    }`}
                />
              ))}
            </div>
